...
/**
 * '게시글 좋아요 취소' 이벤트 발생 시 전달되는 데이터(페이로드)를 담는 클래스입니다.
...
public class ArticleUnlikedEventPayload implements EventPayload {
    ...
    private Long articleId; // 좋아요가 취소된 게시글의 ID
    private Long userId; // 좋아요를 취소한 사용자의 ID
    private LocalDateTime createdAt; // 좋아요를 눌렀던 원래 시각
