...
/**
 * '댓글 삭제' 이벤트 발생 시 전달되는 데이터(페이로드)를 담는 클래스입니다.
...
public class CommentDeletedEventPayload implements EventPayload {
    ...
    private String content; // 삭제된 댓글의 내용
    private String path; // 대댓글의 경우, 상위 댓글들의 ID 경로
    private Long articleId; // 댓글이 달렸던 게시글의 ID
    private Long writerId; // 작성자의 ID
    private Boolean deleted; // 삭제 여부 (삭제 시점에는 항상 true)
